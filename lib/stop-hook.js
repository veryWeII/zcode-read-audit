...
//   2) 生成本轮读取清单(时间/来源/文件/行范围四列表格 + 会话累计行,折叠 <details> 展示),
...
//   数据源三级:①ZCode 原生库 db.sqlite 直查(db-lookup.js,覆盖主会话+子代理,parent_id 精确归属)
//             ②原生库不可用 → 回退本地账本 ledger.jsonl(与 Stop 负载同源,精确匹配)
//             ③账本无该会话(记账 hook 装好前)→ 回退 gryph 时间窗匹配
...
// 「删除手写清单」条款:模型有时会抢在 hook 前自己手写一份预测清单(必错),续写时必须删掉,只保留脚本这份权威清单。
const INTRO_RESTRUCTURE = '【审计系统指令】你的回复已结束,现在输出本回合的最终展示版,无需任何思考或推理,禁止开场白、过渡句或确认。把你刚才的回复重新组织输出:① 2~4 句总体摘要,直接可见;② 若回复中含「需用户注意、需用户决策、建议的下一步计划」类内容,提取成要点单独放入 <details><summary>⚠️ 需注意与下一步</summary>(无此类内容则省略整块);③ 完整原文放入 <details><summary>📖 完整说明</summary>,信息不得删减;④ 重要:若回复中已含你自行手写的「本轮文件读取」清单、累计明细等审计表格,最终展示版必须整体删除这些手写内容——审计数据只以文末本清单为唯一版本;⑤ 最末尾逐字复制下面的审计清单,从 <details> 到 </details>,禁止改写、增删、总结:';
const INTRO_BARE = '【审计系统指令】你的回复已结束,现在做一次纯机械复制续写,无需任何思考或推理。禁止开场白、过渡句、确认或任何其他文字;若你的回复中已含手写的同类清单,一律视为作废。你的下一条输出必须且只能是下面这段清单,从 <details> 逐字复制到 </details>,禁止改写、增删、总结:';
...
function main(payload) {
  ...
  // —— 主数据源(优先):ZCode 原生库直查,覆盖主会话与子代理;任何异常回退本地账本 ——
  let entries = null, cum = null;
  try {
    const r = require('./db-lookup').lookup({ sid, fromMs: sinceMs, toMs: now + 5000, inferShellReads, readRange });
    if (r && r.ok) { entries = r.entries; cum = r.cum; dbg('db lookup ok: entries=' + entries.length + ' (subagent ' + r.subCount + ') cum=' + cum.n); }
    else dbg('db lookup unavailable: ' + ((r && r.error) || 'unknown'));
  } catch (e) { dbg('db lookup failed: ' + e); }

  if (!entries) {
  // —— 回退①:本地账本 ledger.jsonl ——
  entries = []; // {t, src, path, rg},src = 'Read' | 'shell'
  const seen = new Set();
  ...
  if (!ledgerAnyForSid && entries.length === 0) {
    ...
    } catch (e) { dbg('gryph query failed: ' + e); evs = []; }
  ...
  cum = ledgerAnyForSid ? { n: cumN, files: cumFiles.size, rows: cumRows } : null;
  } // —— 回退通道结束,entries/cum 就绪 ——
...
function finish(entries, cum) {
  ...
  if (entries.length === 0) {
    ...
    const rows = entries.slice(0, MAX_ITEMS)
      ...
      .join('\n');
    body = '\n\n<details><summary>📁 本轮文件读取:' + entries.length + ' 个(Read ' + nR + ' · shell ' + nS + (subN ? ' · 🤖子代理 ' + subN : '') + ')</summary>\n\n'
      + '| 时间 | 来源 | 文件 | 范围 |\n|---|---|---|---|\n' + rows + '\n';
