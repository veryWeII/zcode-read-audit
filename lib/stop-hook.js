...
function finish(entries, cum) {
  ...
  const intro = restructureOn()
    ? '【审计系统指令】你的回复已结束,现在输出本回合的最终展示版,无需任何思考或推理,禁止开场白、过渡句或确认。把你刚才的回复与审计清单合并为一条消息重新输出:回复内容在前——原回复较长时,先给 2~4 句总体摘要,再用 <details><summary>📖 完整说明</summary> 折叠完整原文(信息不得删减);原回复较短时直接复读原文。然后在最末尾逐字复制下面的审计清单,从 <details> 到 </details>,禁止改写、增删、总结:'
    : '【审计系统指令】你的回复已结束,现在做一次纯机械复制续写,无需任何思考或推理。禁止开场白、过渡句、确认或任何其他文字,你的下一条输出必须且只能是下面这段清单,从 <details> 逐字复制到 </details>,禁止改写、增删、总结:';
  process.stdout.write(JSON.stringify({ decision: 'block', reason: intro + body }));
}
